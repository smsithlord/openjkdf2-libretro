...
| `retro_serialize_size` | fixed 8 MiB cap. Savestates are the engine's own savegames in an envelope (devdocs/09): serialize = Save Game menu's `Save`+`Process` into a hidden scratch .jks read back into the buffer (empty state when nothing is capturable — the frontend's undo snapshot must not abort loads); unserialize = validate + park `_JKSTATE_PENDING.jks` + arm the Load Game menu's own flow (async, completes over following frames; not-ready loads park in the core and self-arm from `retro_run`). Quirks `INCOMPLETE\|PLATFORM\|ENDIAN` + savestate-context check keep rewind/run-ahead/netplay off this path. Requires the core info file (`savestate_features = "basic"`) AND the DLL in the frontend's cores dir, or RetroArch disables savestates entirely |
