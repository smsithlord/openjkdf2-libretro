...
| `retro_serialize_size` | fixed 8 MiB cap. Savestates are engine-native session data in an envelope (devdocs/09), by mode: **SP** = the engine's own savegame (Save Game menu's `Save`+`Process` into a hidden scratch .jks, read back into the buffer); **MP** = a 428-byte level+pose+character state (the engine cannot save in MP at all), restored through the same teleport MP resume uses. Serialize FAILS when nothing is capturable, so no unloadable slot is written. Unserialize = validate + arm the matching restore (async, completes over following frames); not-ready loads park in the core and self-arm from `retro_run`. Quirks `INCOMPLETE\|PLATFORM\|ENDIAN` + savestate-context check keep rewind/run-ahead/netplay off this path. Requires the core info file (`savestate_features = "basic"`) AND the DLL in the frontend's cores dir, or RetroArch disables savestates entirely |
...
|---|---|---|---|
| `openjkdf2_use_mods` | enabled/disabled | **enabled** | **implemented** — escape hatch only: `mods/*.gob` overriding `resource/` is the normal, upstream behavior, so this defaults ON and merely lets a user play unmodded without moving files. Gates the scan via `jkRes_bAllowModsDir` ([src/Main/jkRes.c](src/Main/jkRes.c)); pre-boot decision, so it needs a content restart. The same scan records a manifest (`jkRes_aModNames`) used for save provenance |
| `openjkdf2_resolution` | 640x480 … 1920x1440 (4:3, 16:9, 16:10) | 640x480 | **implemented** — internal render size. Sets `Window_xSize/ySize` (+ `screen*`) and `Window_resized = 1`, which makes the next engine frame run its own resize path (`jkMain_FixRes` → `stdDisplay_SetMode`, Window.c), and pushes `SET_GEOMETRY` with the new base size + aspect. Applies live; every offered size is within the AV-info maximum so no reinit is needed. Widescreen sizes widen the FOV natively (no stretch/crop) — frontend aspect should be "Core provided" |
| ~~`openjkdf2_autostart_episode`~~ | — | — | **dropped** — fully superseded by `openjkdf2_boot` (`episode`/`level`/`resume` all autostart the ROM's episode; `intro`/`menu` are the opt-outs) |
| ~~`openjkdf2_hires_assets`~~ | — | — | **dropped** — OpenJKDF2 already exposes the hi-res/low-res asset choice in its own in-game options, and the resource scan loads whatever GOBs are present by wildcard; a core option would only duplicate an in-game setting |
...
(`player/<name>/_JKSESSION_<episodeStem>.jks`, a quicksave-shaped
`sithGamesave_Save` + immediate `Process`) at every point that captures a
valid SP pose, and the resume boot loads it through the engine's own
no-world savegame flow (`jkMain_sub_4034D0` → `JK_GAMEMODE_UNK` → gameMode 1
→ `sithGamesave_Restore` — the Load Game menu's cold path), using the save's
own position (no pose teleport). Fallback chain: missing/unreadable/stale
session save (its map must match the pose record's) → pose resume (level
load + teleport; a record without a position resumes at the map's default
spawn) → episode start → title. Users who want a fresh level instead of
full-state restore use `level` mode — that's its role. MP resume stays
pose-only (MP has no saves). The core-drawn menu pointer always auto-hides
(appears on pointer activity, hides after ~3 s idle).
